...
function updateFunctionComponent(fiber) {
  console.log("updateFunctionComponent", fiber); //sy-log
  const { type, props } = fiber;

  const children = [type(props)];
  reconcileChildren(fiber, children);
  console.log("function", fiber);
}
// 暗号：巴拉圭
function updateClassComponent(fiber) {
  ...
  // 执行cmp的render方法，返回一个jsx结构
  let vvnode = cmp.render();
  // 将fiber和这个jsx结构传入reconcileChildren生成完整的fiber结构
  reconcileChildren(fiber, vvnode.props.children);
}
